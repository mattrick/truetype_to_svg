...
namespace tt2svg {
	...
	public:
		...
		std::string svgheader() {
			tmp.str("");
			tmp << "<svg xmlns=\"http://www.w3.org/2000/svg\" height=\"" << face->ascender - face->descender << "\" "
				<< "width=\"" << gm.horiAdvance << "\">";
		...
		std::string outline()  {
			...
			*/
			...
			tmp << "M" << ftpoints[0].x << "," << face->ascender + ftpoints[0].y;
			for (int i = 0 ; i < ftoutline.n_points ; i++) {
				int previndex = i-1;
				int currindex = i;
				int nextindex = i+1;
				...
				if (tags[currindex] & 1) {
					if (tags[previndex] & 1) {
						tmp << " L" << ftpoints[currindex].x << "," << face->ascender + ftpoints[currindex].y;
					}
				} else {
					tmp << " Q" << ftpoints[currindex].x << "," << face->ascender + ftpoints[currindex].y;
					FT_Vector nextp = ftpoints[nextindex];
					if (!(tags[nextindex] & 1)) {
						nextp = halfway_between(ftpoints[currindex], ftpoints[nextindex]);
					}
					tmp << " " << nextp.x << "," << face->ascender + nextp.y;
				}
				if (currindex == contour_endi) {
					...
					tmp << " Z M" << firstp.x << "," << face->ascender + firstp.y;
				}
			}
			tmp << "\" />";
			return tmp.str();
